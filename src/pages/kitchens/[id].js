  ...
  useEffect(() => {
    if (id) {
      const fetchDetails = async () => {
        try {
          const data = await getEntityDetails('kitchens', id);
    ...
    <div className="container mx-auto p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="relative w-full lg:h-[90vh] h-[300px] bg-gray-200 rounded-lg shadow-lg overflow-hidden">
          {kitchenDetails?.image && (
            <Image
              src={`${process.env.NEXT_PUBLIC_API_IMAGE_URL}/${kitchenDetails?.image}`}
              layout="fill"
              objectFit="cover"
              alt={kitchenDetails?.name}
            />
          )}
        </div>
        ...
        <div className="flex flex-col space-y-6">
          <h1 className="text-4xl font-bold text-gray-800">{kitchenDetails?.name}</h1>
          <p className="text-lg text-gray-600">
            <span className="font-semibold">Location: </span> {kitchenDetails?.location}
          </p>
          ...
          <div>
            ...
            <div className="flex space-x-2">
              {kitchenDetails?.operatingShifts.map((shift) => (
                <span
                  key={shift}
                  className={clsx(
                ...
                {kitchenDetails?.operatingShifts.map((shift) => (
                  <TabPanel key={shift}>
                    <div className="bg-white p-4 rounded-lg shadow-md">
                      ...
                      {kitchenDetails?.teams[shift]?.length > 0 ? (
                        <ul className="space-y-2">
                          {kitchenDetails.teams[shift].map((user) => (
                            <li
                              key={user._id}
